...
export default function MaterialRequests({ navigation }) {
  ...
  useEffect(() => {
    const SOCKET_URL = Platform.OS === 'web' 
      ? 'http://localhost:5000' 
      : getBackendUrl(); // Android emulator
...
Delivery: ${quotation.deliveryTerms?.deliveryTime || 'N/A'} days`;
  ...
  const openNegotiationChat = (request) => {
    ...
    if (!quotation) {
      if (Platform.OS === 'web') {
        window.alert('Please create a quotation first before starting negotiation.');
      } else {
        Alert.alert('No Quotation', 'Please create a quotation first before starting negotiation.');
      }
      return;
    }
